...
describe('Mixin', () => {
  ...
  describe('with pgConfig.single = true', () => {
    ...
    it('calls $get when pgConfig.query changed deep', async () => {
      ...
      request.mockClear()
      wrapper.vm.pgConfig.query.and['id.eq'] = 2
      await flushPromises()
    ...
    it('calls $get when pgConfig.route changed', async () => {
      ...
      await flushPromises()
      request.mockClear()
      wrapper.vm.pgConfig.route = 'test'
      await flushPromises()
  ...
  describe('with pgConfig.single = false', () => {
    ...
    it('calls $get when pgConfig.query changed deep', async () => {
      ...
      request.mockClear()
      wrapper.vm.pgConfig.query.or['id.eq'] = 2
      await flushPromises()
  ...
  describe('reactivity', () => {
    ...
    it('creates an reactive pg object', async () => {
      wrapper = shallowMount(
        {
          template: '<pre>{{ pg }}</pre>',
          mixins: [pg],
    ...
    it('creates an reactive pg object for single request', async () => {
      wrapper = shallowMount(
        {
          template: '<pre>{{ pg }}</pre>',
          mixins: [pg],
